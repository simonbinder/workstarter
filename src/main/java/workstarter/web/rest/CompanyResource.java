...
public class CompanyResource {
	...
	public ResponseEntity<Company> createCompany(@Valid @RequestBody Company company) throws URISyntaxException {
		...
		if (company.getId() != null) {
			return ResponseEntity.badRequest().headers(
					HeaderUtil.createFailureAlert(ENTITY_NAME, "idexists", "A new company cannot already have an ID"))
					.body(null);
		}
	...
	/**
	 * SEARCH /_search/companies?query=:query : search for the company
	...
	public List<Company> searchCompanies(@RequestParam String query) {
		...
		return StreamSupport.stream(companySearchRepository.search(queryStringQuery(query)).spliterator(), false)
	 ...
	 *         in body
	 */
	@GetMapping("/companies/{id}/jobadvertisements")
	@Timed
	public List<Jobadvertisment> getAllJobs(@PathVariable Long id) {
		log.debug("REST request to get all Schools for one Student");
		List<Jobadvertisment> jobadvertisments = companyService.getJobs(id);
